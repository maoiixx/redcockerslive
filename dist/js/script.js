			...
			success:function(resp){
				if(resp){
					...
					if(resp.status == 'success'){
						location.replace(_base_url_+'dashboard');
					}else if(resp.status == 'incorrect'){
						var _frm = $('#login-frm')
						var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i>Invalid login credentials</div>"
						_frm.prepend(_msg)
		...
		$.ajax({
			...
			success:function(resp){
				if(resp){
					resp = JSON.parse(resp)
					if(resp.status == 'success'){
						location.replace(_base_url_+'agent');
					}else if(resp.status == 'incorrect'){
						var _frm = $('#flogin-frm')
