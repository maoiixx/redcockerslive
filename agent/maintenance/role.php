        ...
        <div class="container-fluid">
			<table class="table table-stripped table-hover" id="example">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `roles` order by name asc ");
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this permanently?","delete_role",[$(this).attr('data-id')])
		})
		$('.edit_data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Edit Office Details",'maintenance/manage_role.php?id='+$(this).attr('data-id'))
		})
		$('#create_new').click(function(){
			uni_modal("<i class='fa fa-plus'></i> Create New Office",'maintenance/manage_role.php')
		})
	...
	function delete_role($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_role",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
