        ...
        <div class="container-fluid">
			<table class="table table-hover table-stripped" id="example">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `agents`  order by concat(firstname,' ',lastname) asc ");
					...
					?>
						<tr>
							...
							<td><?php echo ($row['type'] == 1) ? 'Administrator' : 'Agent' ?></td>
...
<script>
	...
	function delete_user($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Users.php?f=fdelete",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
