			...
			if($qryBeginingbalance ->num_rows > 0){
				...
				$type = 'Unknown';
                        ...
                        $qry = $conn->query(" select '{$ending_asof}' date_created, '{$ending_amount}' amount, 0 as type, '' accttyp, '' processby
									...
									select convert(date_converted,datetime) date_created, sum(amount) amount,3  as type,
									(select
									concat(case
										when role = 1 then 'Operator'
										when role = 2 then 'Sub-Operator'
									...
									from users where id = a.agent_id) accttyp,
									(select username from users where id = a.agent_id) processby from coms a where active ='N' and user_id = '{$_GET['id']}' and date_converted >= '{$ending_asof}' and date_converted <= CONVERT('2022-09-15 07:00',datetime) group by convert(date_converted,datetime), eventid
