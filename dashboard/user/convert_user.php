...
<div class="container-fluid">
    ...
    <input type="hidden" name ="agent_rate" value="<?php echo $_settings->userdata('rate')?>"> 
        ...
        <input name="rate" id="rate" type="number" inputmode="numeric" pattern="[0-9]*" min="0" max="10.00" step=".1" class="form-control form  rounded-0" value="<?php echo isset($rate) ? $rate : '0.00' ?>" >
        </div>
	</form>
</div>
<script>
$(document).ready(function(){
    $('#transaction-form').submit(function(e){
        //check max rate
        if ($("#rate").val() > 10){
            alert_toast("Max rate is 10!",'error');
			end_loader();
            return false; 
        }
